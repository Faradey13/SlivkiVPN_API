...
export class RegionService {
    ...
    try {
      ...
      const region = await this.prisma.region.create({
        data: {
          ...
          region_name_eng: dto.region_name_eng,
          flag: dto.flag,
        },
      });
  ...
  async getRegionByRusName(regionName: string): Promise<region> {
    try {
      this.logger.info(`Получение региона с Name ${regionName}, проверка кеша...`);
      const cacheKey = `regionName_${regionName}`;
      const regionNameCache = (await this.cacheManager.get(cacheKey)) as region | null;
      if (regionNameCache) {
        this.logger.info(`Регион загружен из кеша: ${JSON.stringify(regionNameCache)}`);
        return regionNameCache;
      }
      const region = this.prisma.region.findFirst({
        where: {
          region_name: {
            equals: regionName,
            mode: 'insensitive',
      ...
      if (region) {
        ...
        this.logger.warn(`Регион с Name ${regionName} не найден в БД`);
      }
    } catch (error) {
      this.logger.error(`Ошибка при получении региона ${regionName}: ${error.message}`);
      throw new Error(`Ошибка при получении региона ${regionName}`);
    }
  }
}
